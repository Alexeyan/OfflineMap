 ...
 */
public abstract class BaseActivity extends MapViewerTemplate implements SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    protected void createTileCaches() {
        boolean persistent = sharedPreferences.getBoolean(SampleApplication.SETTING_TILECACHE_PERSISTENCE, true);
                ...
                builder.setPositiveButton("Ok", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        byte zoomLevel = (byte) ((((SeekBar) view.findViewById(R.id.zoomlevel)).getProgress()) +
                                BaseActivity.this.mapView.getModel().mapViewPosition.getZoomLevelMin());

                        BaseActivity.this.mapView.getModel().mapViewPosition.setMapPosition(
    ...
    protected void onPrepareDialog(int id, final Dialog dialog) {
        if (id == DIALOG_ENTER_COORDINATES) {
            IMapViewPosition currentPosition = BaseActivity.this.mapView.getModel().mapViewPosition;
            ...
            zoomlevel.setProgress(BaseActivity.this.mapView.getModel().mapViewPosition.getZoomLevel()
    ...
    public void onSharedPreferenceChanged(SharedPreferences preferences, String key) {
        if (SampleApplication.SETTING_SCALE.equals(key)) {
            this.mapView.getModel().displayModel.setUserScaleFactor(DisplayModel.getDefaultUserScaleFactor());
            Log.d(SampleApplication.TAG, "Tilesize now " + this.mapView.getModel().displayModel.getTileSize());
            AndroidUtil.restartActivity(this);
        }
        if (SampleApplication.SETTING_PREFERRED_LANGUAGE.equals(key)) {
            String language = preferences.getString(SampleApplication.SETTING_PREFERRED_LANGUAGE, null);
            Log.d(SampleApplication.TAG, "Preferred language now " + language);
            AndroidUtil.restartActivity(this);
        }
        if (SampleApplication.SETTING_TILECACHE_PERSISTENCE.equals(key)) {
            if (!preferences.getBoolean(SampleApplication.SETTING_TILECACHE_PERSISTENCE, false)) {
                Log.d(SampleApplication.TAG, "Purging tile caches");
        ...
        if (SampleApplication.SETTING_TEXTWIDTH.equals(key)) {
            AndroidUtil.restartActivity(this);
        }
        if (SETTING_SCALEBAR.equals(key)) {
            setMapScaleBar();
        }
        if (SampleApplication.SETTING_DEBUG_TIMING.equals(key)) {
            MapWorkerPool.DEBUG_TIMING = preferences.getBoolean(SampleApplication.SETTING_DEBUG_TIMING, false);
        }
        if (SampleApplication.SETTING_RENDERING_THREADS.equals(key)) {
            Parameters.NUMBER_OF_THREADS = preferences.getInt(SampleApplication.SETTING_RENDERING_THREADS, 1);
            AndroidUtil.restartActivity(this);
        }
        if (SampleApplication.SETTING_WAYFILTERING_DISTANCE.equals(key) ||
                SampleApplication.SETTING_WAYFILTERING.equals(key)) {
            MapFile.wayFilterEnabled = preferences.getBoolean(SampleApplication.SETTING_WAYFILTERING, true);
            if (MapFile.wayFilterEnabled) {
                MapFile.wayFilterDistance = preferences.getInt(SampleApplication.SETTING_WAYFILTERING_DISTANCE, 20);
            }
        }
    }
    ...
    protected void setMaxTextWidthFactor() {
        mapView.getModel().displayModel.setMaxTextWidthFactor(Float.valueOf(sharedPreferences.getString(SampleApplication.SETTING_TEXTWIDTH, "0.7")));
    }
